...
import ComingStyle from './styles.js';
...
export default function ComingSoon({navigation}) {
    ...
    const fetchPage = (url, existingMenus) => {
        ...
        getComingMenus(url)
            .then((response) => {
                if(!response.Error){
                    setComingMenus(existingMenus.concat(response.result));
    ...
    const renderize = ({item}) => <ComingMenu menu={item}/>;
    
    //Si no hay menus renderiza un mensaje
    const emptyRenderize = () => <Text style={[ComingStyle.emptyList]}>No hay menus disponibles</Text>;
        ...
        <View style={ComingStyle.background}>
            ...
            <View style={ComingStyle.container}>
                <FlatList
                    ...
                    refreshing={reload}
                    onRefresh={refresh}
                    keyExtractor={item => JSON.stringify(item.fecha)}
